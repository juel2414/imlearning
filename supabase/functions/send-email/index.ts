      ...
      <tr>
        <td style="padding:18px 32px 26px;border-top:1px solid #eee;text-align:center;">
          <p style="margin:0;color:#aaa;font-size:11px;line-height:1.8;">
            ...
            <span style="color:#ccc;">© 2025 아이엠러닝. All rights reserved.</span>
          </p>
        </td>
      </tr>
    </table>
  ...
  return base(`
    ...
    <div style="background:#f0faf5;border-radius:10px;padding:20px 24px;margin-bottom:28px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="padding:5px 0;color:#333;font-size:13px;">📚</td>
          <td style="padding:5px 0;color:#333;font-size:13px;">검정고시 전 과목 (중졸·고졸)</td>
        </tr>
        <tr>
          <td style="padding:5px 0;color:#333;font-size:13px;">🌎</td>
          <td style="padding:5px 0;color:#333;font-size:13px;">토익 입문 ~ 중급 (왕기초 포함)</td>
        </tr>
        <tr>
          <td style="padding:5px 0;color:#333;font-size:13px;">✝️</td>
          <td style="padding:5px 0;color:#333;font-size:13px;">신앙 성장 & 부모·선교 교육</td>
        </tr>
        <tr>
          <td style="padding:5px 0;color:#333;font-size:13px;">🎓</td>
          <td style="padding:5px 0;color:#333;font-size:13px;">리뉴젠아카데미 전 과목</td>
        </tr>
      </table>
    </div>
  ...
  return base(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8e8e8;border-radius:10px;overflow:hidden;margin-bottom:28px;">
      <tr style="background:#fafafa;">
        <td style="padding:13px 18px;color:#888;font-size:12px;font-weight:600;width:36%;border-bottom:1px solid #eee;">강좌명</td>
        <td style="padding:13px 18px;color:#222;font-size:13px;font-weight:700;border-bottom:1px solid #eee;">${courseName}</td>
      </tr>
      <tr>
        <td style="padding:13px 18px;color:#888;font-size:12px;font-weight:600;border-bottom:1px solid #eee;">결제 금액</td>
        <td style="padding:13px 18px;color:${BRAND};font-size:18px;font-weight:900;border-bottom:1px solid #eee;">₩${amtStr}</td>
      </tr>
      <tr style="background:#fafafa;">
        <td style="padding:13px 18px;color:#888;font-size:12px;font-weight:600;">결제일시</td>
        <td style="padding:13px 18px;color:#444;font-size:13px;">${dateStr}</td>
      </tr>
    </table>
    <div style="text-align:center;">
...
function completionHtml(courseName: string) {
  ...
  return base(`
    ...
    <p style="margin:0;color:#888;font-size:13px;text-align:center;line-height:1.7;">더 많은 강좌에 도전해보세요!</p>
...
function newCourseHtml(courseTitle: string, courseId: string | number, thumbnailUrl?: string) {
  ...
  return base(`
    <div style="text-align:center;">
      ...
      <p style="margin:0 0 28px;color:#666;font-size:13px;line-height:1.7;">
        아이엠러닝에 새 강좌가 추가됐습니다.<br>지금 바로 확인해보세요!
      </p>
...
Deno.serve(async (req: Request) => {
  ...
  const err = (status: number, msg: string) => new Response(JSON.stringify({ error: msg }), {
  ...
  try {
    ...
    // ── 1. 회원가입 환영 메일 (인증 불필요 — 가입 직후 세션 없음) ──
    if (type === 'welcome') {
    ...
    const userName = (profile?.name as string) || '회원'
