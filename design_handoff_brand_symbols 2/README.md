...
# 추가 세트: 학습 성취 배지 11종 (Learning Badges)

기존 브랜드 심볼과 **완전히 동일한 construction spec·글로시 규칙**으로 제작한 성취 배지 세트입니다.
성취도가 낮은 것 → 높은 것 순으로 갈수록 크기·톤이 진해져 **레벨/위계**가 시각적으로 드러납니다.

## New tokens (배지용 신규 토큰)
기존 9개 토큰과 색상환에서 겹치지 않게 조정했습니다.

| 토큰 | Top | Bottom | 사용처 |
|---|---|---|---|
| flame3 | `#FF9A5C` | `#E85D2C` | badge-streak-3 (3일 연속) — 따뜻한 레드-오렌지, gold/coral과 구분 |
| flame7 | `#FF7A3C` | `#D2400F` | badge-streak-7 (7일 연속) — flame3보다 한 단계 진한 톤(위계) |
| bolt30 | `#FFD84D` | `#8A3CE0` | badge-streak-30 (30일 연속) — 전기적 옐로우→퍼플, 최상위 에너지 |
| mint | `#7DE8B0` | `#12B074` | badge-goal-achieved — teal/green과 구분되는 민트 |
| growth | `#A6E06A` | `#4FA524` | badge-above-avg — 성장 지표용 라임-그린 |

재사용 토큰: blue(lessons-10), gold(multi-graduate, passion, top-10), green(first-step, graduate).
...
| 파일 | 한글 | 조건/용도 | 색상 | 비고 |
|---|---|---|---|---|
| `badge-first-step.svg` | 첫 걸음 | 첫 강의 수강 시작 | green | `sprout.svg` 재사용(복사본) |
| `badge-streak-3.svg` | 3일 연속 | 3일 연속 학습 | flame3 | 작은 불꽃 |
| `badge-streak-7.svg` | 7일 연속 | 7일 연속 학습 | flame7 | 더 크고 진한 불꽃 |
| `badge-streak-30.svg` | 30일 연속 | 30일 연속 학습 | bolt30 | 번개(에너지 최상위) |
| `badge-lessons-10.svg` | 10강 달성 | 누적 10강 완료 | blue | 책 스택(book.svg와 구도 구분) |
| `badge-goal-achieved.svg` | 오늘 목표 달성 | 일일 목표 달성 | mint | 과녁 + 체크 |
| `badge-graduate.svg` | 수료 | 강좌 수료 | green | `completion.svg` 재사용(복사본) |
| `badge-multi-graduate.svg` | 멀티 수료 | 2개 이상 수료(상위) | gold | 왕관(상위 등급) |
| `badge-passion.svg` | 열정왕 | 30개 이상 수강 | gold | 광채+스파클 별(star.svg보다 화려) |
| `badge-above-avg.svg` | 평균 초과 학습 | 평균 초과 학습량 | growth | 상승 막대그래프 |
| `badge-top-10.svg` | 상위 10% | 평점/성취 상위 10% | gold | `trophy.svg` 재사용(복사본) |

**위계 진행**: streak 3 < 7 < 30 (불꽃→큰불꽃→번개), first-step < lessons-10 < passion (새싹→책스택→광채별).
재사용 3종(first-step/graduate/top-10)은 원본(sprout/completion/trophy)을 **그대로 두고 배지명 복사본**으로 별도 파일화했습니다.

## Badge preview
- `preview/IMlearning Badge Set.dc.html` — 11종 위계 비교 미리보기(참고용)
